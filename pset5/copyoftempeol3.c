...
/* ################################################# STRUCT DEF ################################################################## */     
 ...
 /* ################################################# FUNCTIONS ################################################################## */   
    ...
    void read_dictionary(FILE* dict, char oneword[], node* hashtable[], node* head[])
    {       
...
/* ################################################# FUNCTIONS END :: MAIN START ################################################################## */    
    ...
    {            
        ...
        node* hashtable [26];       //1. creates a hashtable and heads
        node* head [26];
                
        init_table(hashtable, head);      //1. initialize hashtable to NULL
        
        read_dictionary(dict, oneword, hashtable, head);      
        ...
        {               
            if(c == ' ' || c == '\n' || c == '\0' || c == '\t' || c == '!' || c == '.' || c == ',' || c == '?' || c == ')' || c == ':' || c == '-' || c == '(' || c =='&')
            {
                ...
                int flag = 0;
                int hash1 = findhash(checkword[0]);
                //printf("%d\n",hash1);
